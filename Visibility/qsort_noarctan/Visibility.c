...
*/
Horizon* visibility(int start, int end, Point** points, int* numVisible, int rightSide) {
  assert(points);
  ...
  Horizon* h1 = visibility(start, (start+end)/2, points, numVisible, rightSide);
  Horizon* h2 = visibility((start+end)/2, end, points, numVisible, rightSide);
