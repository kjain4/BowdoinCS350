...
vappend(Vector *v, const vector_data_t e) {
  ...
  memcpy(((char*)v->data + v->length * v->data_size), e, v->data_size);
  v->length++;
}
...
vappend_vector(Vector *v1, const Vector *v2) {
  ...
  memcpy((char*)v1->data + v1->length * v1->data_size, v2->data,
...
vset(const Vector *v, vector_size_t index, const vector_data_t e) {
  ...
  memmove((char*)v->data + index * v->data_size, e, v->data_size);
}
...
vindex(const Vector *v, const vector_data_t e) {
  ...
  ptr = v->data;
  end = (char*)v->data + v->length * v->data_size;
  while (ptr < end) {
    if (memcmp(ptr, e, v->data_size) == 0)
      return ((char*)ptr - (char*)v->data) / v->data_size;
    ptr = (char*)ptr + v->data_size;
  }
  return v->length;
}
...
vremove(Vector *v, vector_size_t index) {
  ...
  if (index < v->length - 1)
    memmove((char*)v->data + index * v->data_size,
            (char*)v->data + (index + 1) * v->data_size,
...
vpop(Vector *v) {
  ...
  v->length--;
  return (char*)v->data + v->length * v->data_size;
}
